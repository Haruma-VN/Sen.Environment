namespace Sen.Script.Helper.PvZ2.Animation.GenerateAtlasFromData {
    ...
    export function process(atlas: Support.PopCap.Atlas.Structure.Definition, record: Support.PopCap.Animation.Miscellaenous.GenerateData.Data): void {
    ...
    export function process_fs(): void {
        const record = Console.path(Kernel.Language.get("script.helper.pvz2.animation.generate_atlas_from_data.input_data_json"), "file");
        Console.send(Kernel.Language.get("script.helper.pvz2.animation.generate_atlas_from_data.input_subgroup_name"), Definition.Console.Color.CYAN);
        const subgroup = Kernel.Console.readline();
        const resolutions = get_resolution();
        if (resolutions.length === 0) {
            throw new Error(format(Kernel.Language.get("script.helper.pvz2.animation.generate_atlas_from_data.resolution_not_found"), allowance));
        }
        ...
        resolutions.forEach(function handle_resolution(resolution) {
            ...
            process(atlas, Kernel.JSON.deserialize_fs<Support.PopCap.Animation.Miscellaenous.GenerateData.Data>(record));
