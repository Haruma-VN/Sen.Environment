namespace Sen.Script.Support.PopCap.Animation {
    export namespace ToAnimation {
        ...
        export function process(source: string): SexyAnimation {
            ...
            const animation_image_id_list: string[] = Kernel.FileSystem.read_directory_only_file(
                Kernel.Path.join(source, "library", "image"))
                .filter(e => /(.*)\.xml$/gi.test(e)).map(e => Kernel.Path.base_without_extension(e));

            debug_json(animation_image_id_list);
            ...
            const animation_sprite_name_list: string[] =
                Kernel.FileSystem.read_directory_only_file(
                    Kernel.Path.join(source, "library", "sprite")).filter(e => /(.*)\.xml$/gi.test(e)).map(e => Kernel.Path.base_without_extension(e));
        ...
        export function debug_json(o: any) {
            Kernel.Console.print(Kernel.JSON.serialize(o, 1, false));
            return;
        }

        export function w_json(p: string, o: any) {
            Kernel.JSON.serialize_fs(p, o, 1, false);
            return
        }
        ...
        export function parse_transform_for_image(matrix: Matrix): number[] {
            ...
            return [
                ...
                parseFloat(matrix["@attributes"]["c"] ?? "0"),
                d_matrix,
