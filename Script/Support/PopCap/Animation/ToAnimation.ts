namespace Sen.Script.Support.PopCap.Animation {
    export namespace ToAnimation {
        ...
        export function process(source: string): SexyAnimation {
            const extra_info: ExtraInfo = Kernel.JSON.deserialize_fs(Kernel.Path.join(source, "extra.json"));
            const animation_image_id_list: string[] = sort_list(Kernel.FileSystem.read_directory_only_directory(Kernel.Path.join(source, "library", "image")));
            ...
            for (let image_id of animation_image_id_list) {
                ...
                animation_image_map[image_id] = parse_image(image_document, image_id, extra_info);
            }
            const animation_sprite_name_list: string[] = sort_list(
                Kernel.FileSystem.read_directory_only_directory(Kernel.Path.join(source, "library", "sprite"))
            );
            ...
            const animation_label_list: string[] = Kernel.FileSystem.read_directory_only_directory(Kernel.Path.join(source, "library", "action"));
            const animation_action_map: Record<string, FrameNode> = {};
            for (let action_label of animation_label_list) {
                const action_document: SpriteDocument = Kernel.XML.deserialize_fs(Kernel.Path.join(source, "library", "sprite", `${action_label}.xml`));
        ...
        export function parse_dom_document(
            dom_document: DOMDocument,
            animation: Structure.SexyAnimation,
            action_index_list: Record<string, bigint>,
            last_frame: bigint
        ): void 
        {
        ...
        export function parse_sprite(
            sprite_document: SpriteDocument,
            sprite_name: string,
            is_action: boolean,
            animation_image_id_list: string[],
            ...
            for (let layer of sprite_layers_list) {
                ...
                for (let frame of sprite_frames_list) {
                    ...
                    if (!frame["DOMFrame"]["elements"].hasOwnProperty("DOMSymbolInstance")) {
                        throw new Error("sprite_has_no_DOMSymbolInstance");
                    }
                    const dom_symbol_instance: DOMSymbolInstance | null = frame["DOMFrame"]["elements"]["DOMSymbolInstance"];
                    if (dom_symbol_instance === null) {
                        if (close_current_model(frames_result, model)) model = null;
                        break;
                    }
                    ...
                    const current_instance: Instance = {
                        resource: is_sprite
                            ? BigInt(animation_sprite_name_list.indexOf(sprite_item_name))
                            : BigInt(animation_image_id_list.indexOf(sprite_item_name)),
                        sprite: is_sprite,
                    };
                    let transform: number[];
                    if (
                        dom_symbol_instance["matrix"] === undefined ||
                        dom_symbol_instance["matrix"] === null ||
                        Object.keys(dom_symbol_instance["matrix"]).length === 0
                    ) {
                        transform = [0.0, 0.0];
                    ...
                    let color: number[];
                    if (
                        dom_symbol_instance["color"] === undefined ||
                        dom_symbol_instance["color"] === null ||
                        Object.keys(dom_symbol_instance["color"]).length === 0
                    ) {
                    ...
                    const target_frame: Structure.AnimationFrame = get_frame_at_index(frames_result, frame_index);
                    if (model === null) {
                        model = {
                            state: false,
                ...
                if (close_current_model(frames_result, model)) {
                    model = null;
                }
            }
        ...
        export function get_frame_at_index(frames_result: Structure.AnimationFrame[], index: bigint): Structure.AnimationFrame {
        ...
        export function close_current_model(frames_result: Structure.AnimationFrame[], model: Model | null): boolean {
            if (model !== null) {
                const target_frame: Structure.AnimationFrame = get_frame_at_index(frames_result, model["frame_start"] + model["frame_duration"]);
        ...
        export function parse_image(image_document: ImageDocument, image_id: string, extra_info: ExtraInfo): Structure.AnimationImage {
        ...
        export function parse_transform_for_image(matrix: Matrix): number[] {
            const resolution_list: BigInt[] = [1200n, 1536n, 768n, 384n, 640n];
            let a_matrix: number = parseFloat(matrix["@attributes"]["a"] ?? "1");
            if (resolution_list.includes(BigInt(a_matrix * 1200))) {
                a_matrix = 1;
            }
            let d_matrix: number = parseFloat(matrix["@attributes"]["d"] ?? "1");
            if (resolution_list.includes(BigInt(d_matrix * 1200))) {
                d_matrix = 1;
            }
            return [
                a_matrix,
        ...
        export function process_fs(source: string, desitnation: string): void {
            Kernel.JSON.serialize_fs("C:/Users/Shift/Desktop/123/PRIMAL_SUNFLOWER_2.PAM.XFL/1.json", Kernel.XML.deserialize_fs("C:/Users/Shift/Desktop/123/PRIMAL_SUNFLOWER.PAM.XFL/DOMDocument.xml"), 1, false);
